...
#pragma pack( 1 )
	...
	union
	{
		...
		// and are we wrong?
		int* m_CMatrixPre[4];			// 16, a part of CPlaceable
		float* preMatrix;				// 16, a part of CPlaceable
		int* preMatrixStruct[4];		// 16, a part of CPlaceable
	};
	union
	{
		int* m_CMatrix[4];				// 20
		float* matrix;				// 20
		int* matrixStruct[4];			// 20
	};
...
#pragma pack( 1 )
	...
	int			pedFlags[4];			/* 1132 */
	int* pPedIntelligence; /* 1148 */
	int* pPlayerData; /* 1152 */
	...
	*/
	...
	int					pedIK[8];	/* 1292 - Inverse Kinematics */
	...
	union
	{
		int		* pContactEntity;	/* 1384 - touching a CEntitySAInterface */
...
#pragma pack( 1 )
	...
	int							m_nVehicleFlags[2];	/* 1064 - 60 bits/flags */
